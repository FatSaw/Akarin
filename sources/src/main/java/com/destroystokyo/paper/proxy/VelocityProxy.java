...
public class VelocityProxy {
    ...
    public static InetAddress readAddress(final PacketDataSerializer buf) {
        return InetAddresses.forString(buf.e(Short.MAX_VALUE));
    }

    public static GameProfile createProfile(final PacketDataSerializer buf) {
        final GameProfile profile = new GameProfile(buf.i(), buf.e(16));
        readProperties(buf, profile);
        return profile;
    }

    private static void readProperties(final PacketDataSerializer buf, final GameProfile profile) {
        final int properties = buf.g();
        for (int i1 = 0; i1 < properties; i1++) {
            final String name = buf.e(Short.MAX_VALUE);
            final String value = buf.e(Short.MAX_VALUE);
            final String signature = buf.readBoolean() ? buf.e(Short.MAX_VALUE) : null;
